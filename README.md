...
| `udp_port` | UDP 监听端口（音箱向网关发发现请求用） | `6666` |
| `log_level` | 日志级别 | `info` |

> ⚠️ 如果端口被占用，请修改为其他值（如 `18888`、`16666`）
> ⚠️ `localtuya` 默认会监听 UDP `6666/6667`，不能与声必可网关共用同一个 UDP 监听端口；如需共存，请修改本加载项的 `udp_port` 或关闭 localtuya 的发现监听。
> ℹ️ 音响接收网关 UDP 广播/响应的端口是固定 `7777`，无需配置。
...
Web 配置界面已启用登录保护。首次启动时使用默认账号登录：

- 用户名：`admin`
- 初始密码：`admin`

首次登录后必须立即修改密码；修改完成后请使用新密码登录。认证数据会持久化保存到 `/app/data`，Docker 部署时请确保该目录位于持久化卷中。登录页面提供退出登录功能。
...
- 网关会向音响接收端口 `7777` 主动广播连接信息，目标为 `255.255.255.255` 和各网卡子网广播地址，音箱无需手动配置 IP
</details>
...
- 如 HA 安装了 `localtuya`，它默认监听 UDP `6666/6667`，会和声必可网关默认发现端口冲突
